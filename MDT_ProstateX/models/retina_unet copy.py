...
def compute_class_loss(anchor_matches, class_pred_logits, shem_poolsize=20):
    ...
    # Also, anchors of special class_id == 21 do not contribute either
    matches_to_consider= (anchor_matches > 0) & (anchor_matches != 21)
    pos_indices = torch.nonzero(matches_to_consider)
...
def compute_bbox_loss(target_deltas, pred_deltas, anchor_matches):
    ...
    """
    #We want to detect all matches, even for unknown class 21
    matches_to_consider= (anchor_matches > 0)
    if 0 not in torch.nonzero(matches_to_consider).size():

        indices = torch.nonzero(matches_to_consider).squeeze(1)
...
def refine_detections(anchors, probs, deltas, batch_ixs, cf):
    ...
    keep_arr = torch.cat((
        torch.div(keep_ix, fg_probs.shape[1], rounding_mode='trunc').unsqueeze(1),
        (keep_ix % fg_probs.shape[1]).unsqueeze(1)
    ), 1)
