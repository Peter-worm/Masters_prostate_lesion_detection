...
class ModelSelector:
    ...
    def run_model_selection(self, net: torch.nn.Module, optimizer: torch.optim.Optimizer,
        ...
        # check if current epoch is among the top-k epochs or self.cf.keep_all_epochs is True
        if epoch in epoch_ranking[:self.cf.save_n_models] or self.cf.keep_all_epochs:
            ...
            if not self.cf.keep_all_epochs:
